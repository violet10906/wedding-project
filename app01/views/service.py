...
from app01.models import ServiceCar
...
def service_list(request):
    ...
    if search_data:
        data_dict["typeCar__contains"]=search_data

    page = int(request.GET.get("page", "1"))  # 当前页
    page_size = 10  # 每页显示10条数据
    start = (page - 1) * 10
    end = page * 10
    total_count = ServiceCar.objects.filter(**data_dict).order_by("-id").count()  # 数据总条数
    ...
    queryset = ServiceCar.objects.filter(**data_dict)[start:end]

    return render(request,"service_list.html",{"queryset":queryset,"seaerch_data":search_data,"page_string":page_string})

class ServiceForm(Bootstrap):
    class Meta:
        model=ServiceCar
        fields="__all__"
...
def service_edit(request,id):
    '''修改服务'''
    data=ServiceCar.objects.filter(id=id).first()
    if request.method=="GET":
        form=ServiceForm(instance=data)
        return render(request,"change.html",{"form":form})
    form=ServiceForm(data=request.POST,instance=data)
    if form.is_valid():
        form.save()
        return redirect("/service/list/")
    return render(request,"change.html",{"form":form})
...
def service_delete(request,id):
    '''删除服务'''
    ServiceCar.objects.filter(id=id).delete()
    return redirect("/service/list/")
